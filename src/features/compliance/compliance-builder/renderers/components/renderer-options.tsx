...
export default function RendererOptions({ scope = "top", ...props }: OptionsProps) {
	...
	const container = classNames(
		...
		{
			"flex-col top-0 -right-10  bg-white border-black": scope === "side",
			"flex-row -top-[3.4rem] right-6 bg-neutral-900": scope === "top",
			"flex-row -top-3.5 right-0 bg-neutral-900 group-hover:opacity-100 opacity-0": scope === "field",
		}
	);
