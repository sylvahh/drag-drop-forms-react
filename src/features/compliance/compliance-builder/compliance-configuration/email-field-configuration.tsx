...
export default React.memo(function EmailFieldConfiguration() {
				...
				for (const formBlock of module.form_blocks) {
					for (const row of formBlock.field_rows) {
						const f = row.fields.find(
							(f) => f.id === selectedConfigurationType?.id && f.type === "email"
						);
						if (f) {
	...
	if (selectedConfigurationType?.type !== "email") return null;
	return (
