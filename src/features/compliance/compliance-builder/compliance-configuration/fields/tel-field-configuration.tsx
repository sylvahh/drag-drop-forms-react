...
export default React.memo(function TelFieldConfiguration() {
				...
				for (const formBlock of module.form_blocks) {
					for (const row of formBlock.field_rows) {
						const f = row.fields.find(
							(f) => f.id === selectedConfigurationType?.id && f.type === "tel"
						);
						if (f) {
	...
	if (selectedConfigurationType?.type !== "tel") return null;
	return (
