...
export default React.memo(function RadioFieldConfiguration() {
				...
				for (const formBlock of module.form_blocks) {
					for (const row of formBlock.field_rows) {
						const f = row.fields.find(
							(f) => f.id === selectedConfigurationType?.id && f.type === "radio"
						);
						if (f) {
	...
	if (selectedConfigurationType?.type !== "radio") return null;
	return (
