...
export default React.memo(function CreatedAtFilter(props: FilterFormProps) {
	const [from, to] = props.formData.created_at.split(" - ");
	...
	React.useMemo(() => {
		...
		props.change("created_at", `${dateRange.from} - ${dateRange.to}`);
	...
	React.useMemo(() => {
		if (props.formData.created_at) return;
		setDateRange(init);
	}, [props.formData.created_at]);

	return (
		<div>
			<span className="mb-1 text-gray-500 text-b-2">Created At</span>
